...
function reloadImage(callbacks) {
  return Jimp.read(imagePaths.secretPath).then(function(secretPic) {
    // console.log("Reading secret image!");

    Jimp.read(imagePaths.mainPath).then(function(releasedPic) {
      ...
      PixelSchema.find({})
        ...
        .on("data", function(pixelObj) {
          var hex = pixelObj.isBought
            ? secretPic.getPixelColor(pixelObj.pixel.x, pixelObj.pixel.y)
            : 0xffffffff;
...
var PixelHandler = {
  ...
  buyPixels: function(userID, message, amount, callbacks) {
    return PixelSchema.findRandom(
      { isBought: false },
      {},
      { limit: amount },
      function(err, pixelObjs) {
        if (err) {
          console.error(err);
